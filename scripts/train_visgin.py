"""
Script: Train VisGIN — Visibility Graph Neural Network
======================================================
Trains the VisGIN model with physical keypoint distance-based attention modulation.
Saves model checkpoint to outputs/visgin/best_model.pt.
"""
...
from src.models.visgin import CattleVisGIN
...
def main():
    ...
    dropout       = 0.25
    use_amp       = True
    ckpt_dir      = str(PROJECT_ROOT / 'outputs/visgin')
    patience      = 50
    min_delta     = 0.001
    label_smooth  = 0.1
    ...
    print(f"\n{'='*65}")
    print("  VisGIN (Visibility GNN) TRAINING")
    print(f"{'='*65}")
    ...
    print(f"  Dropout: {dropout} | Label Smooth: {label_smooth}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in loaders['train']:
            ...
            with autocast(device_type='cuda', dtype=amp_dtype, enabled=use_amp):
                ...
                if label_smooth > 0 and 'embedding' in out:
                    emb = out['embedding']
                    smooth_loss = -(emb.mean(dim=1).pow(2).mean()) * label_smooth
    ...
    # -- Final Evaluation --
    print(f"\n{'='*65}")
    print(f"VisGIN Training Complete! Best R1: {best_r1:.4f} @ epoch {best_epoch}")
    print(f"{'='*65}")
    ...
    save_stats({
        'model': 'VisGIN',
        'architecture': 'VisGIN (Visibility-based Graph Attention Network)',
    ...
    }, str(PROJECT_ROOT / 'outputs/stats/visgin_results.json'))

    print(f"\nVisGIN results saved to outputs/stats/visgin_results.json")
