...
measurable mis-calibration signal (the baseline equal-error rate). In a controlled comparison on
...
MegaDescriptor-L & panda pattern & natural (cross-video) & 2.63\% & 2.29\% & $+0.3$ pt & $[-0.33,+0.14]$ \textbf{n.s.} \\
...
\section{Results}
Table~\ref{tab:main} summarizes recovery across backbones, modalities, and shift families.

\textbf{The conditional law.} Recovery magnitude and significance track the baseline EER. Where the
