...
\begin{table}[t]
  \centering
  \caption{5-fold cross-validation performance. The Hybrid is now trained to
  \emph{convergence} (150 cached + 30 end-to-end epochs per fold); the CNN and ProtoN rows
  use a reduced budget (10 and 12 epochs), which is sufficient for those two models but
  leaves their numbers slightly conservative.}
  \label{tab:cv}
  \small
  ...
  \begin{tabular}{lccccc}
    ...
    \midrule
    \textbf{Hybrid CNN--GNN} & full & \textbf{95.52 $\pm$ 0.31} & \textbf{97.40 $\pm$ 0.27} & \textbf{0.49 $\pm$ 0.16} & \textbf{0.9998} \\
    CNN (EfficientNet-B4) & reduced & 93.91 $\pm$ 0.31 & 96.65 $\pm$ 0.45 & 3.21 $\pm$ 0.22 & 0.9940 \\
    ProtoN GNN & reduced & 89.49 $\pm$ 0.71 & 93.31 $\pm$ 0.52 & 4.10 $\pm$ 0.71 & 0.9911 \\
    \midrule
    \emph{Hybrid (12 ep., superseded)} & reduced & \emph{68.88 $\pm$ 2.04} & \emph{84.22 $\pm$ 1.27} & \emph{11.31 $\pm$ 1.47} & \emph{0.9491} \\
    \bottomrule
  \end{tabular}
\end{table}

Our earlier cross-validation used a reduced budget for all models to keep the sweep
...
that the architecture is unstable. We have since re-run the Hybrid at \emph{full budget}
(150 cached + 30 end-to-end epochs per fold, $\approx$82 minutes per fold on one RTX 5070).
The result settles the question: Rank-1 rises from $68.88\pm2.04\%$ to
$\mathbf{95.52\pm0.31\%}$ and EER falls from $11.31\%$ to $\mathbf{0.49\pm0.16\%}$, with the
fold variance collapsing by a factor of six. The earlier number was an artefact of
under-training, not instability, and the Hybrid generalises stably across folds.

\paragraph{Why the cross-validated Hybrid exceeds its single-split value.} The full-budget CV
Rank-1 ($95.52\%$) is higher than the Hybrid's single-split test result ($92.0\%$,
\Cref{tab:main}), and we state the reasons rather than leave the discrepancy implicit. First,
each fold trains on $80\%$ of the data versus $70\%$ for the single split --- roughly $14\%$
more training images. Second, and more importantly, the two protocols select their final model
differently: each CV fold trains for a fixed schedule and evaluates the \emph{final} model,
whereas the single-split model is the best-validation-Rank-1 checkpoint, chosen on a small
...
selection, so both numbers are honest; they simply answer different questions. We therefore
keep \Cref{tab:main} (single split, matched to every other single-split result in this paper)
as the headline and read \Cref{tab:cv} as evidence of \emph{stability across folds}. McNemar
tests on single-split predictions confirm the CNN's advantage over the pure GNNs is
significant ($p<10^{-8}$).
...
% ===========================================================================
(i) The CNN and ProtoN rows of \Cref{tab:cv} still use a reduced training budget, so those
two cross-validated numbers are conservative relative to their converged single-split values;
the Hybrid row is full-budget. (ii) We
...
condition. (v) The two GNN explainers we compare show
low mutual rank agreement, so per-keypoint importance is indicative rather than definitive.
(vi) The primary split is image-level: the dataset ships no session, capture-date, or camera
