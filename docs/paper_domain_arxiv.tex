...
(3.5$\times$ better verification). A control the literature usually omits, however, bounds what that means: fusing the same CNN with a \emph{non-graph} baseline (VGG-16) reduces EER by a statistically \emph{indistinguishable} amount (CIs $[+0.47,+2.63]$ vs $[+0.44,+2.48]$ points), so the verification gain is not graph-specific --- it reflects a poorly calibrated CNN that almost any better-calibrated partner repairs. What remains graph-specific is narrower, and we state it as such: of the partners tested, only the graph branch improves ranking \emph{and} verification simultaneously. A pathway-intervention study then sharpens
...
\begin{enumerate}\itemsep2pt
  ...
  sampling variant.
  \item \textbf{Where graphs help --- and, just as importantly, where they do not.} Rather than claiming graphs beat CNNs on ranking, we bound the graph branch's contribution with two controls that comparable studies typically omit. (i) A validation-selected CNN+Hybrid blend cuts the Equal Error Rate 3.5$\times$ (2.70\%$\to$0.78\%) --- but fusing the same CNN with a \emph{non-graph} baseline achieves a statistically indistinguishable reduction, so the verification gain is \emph{not} graph-specific (\Cref{tab:fusioncontrol}). What is specific is narrower: only the graph partner improves ranking and verification at once. (ii) A pathway intervention shows the gain does \emph{not} come from the explicit geometric topology --- randomising the edges consumed by the GATv2 relation module is nearly free, while removing the keypoint-sampled node features is catastrophic. Together these are an instructive negative result for graph-based biometrics.
...
\paragraph{Is the verification gain graph-specific? A control.} The CNN's 2.70\% EER is the weakest of our strong models --- VGG-16 reaches 1.23\% and ProtoN 1.17\% standalone --- so a natural objection is that the blend simply repairs a poorly calibrated CNN, with the graph incidental. We test this directly by running the \emph{identical} validation-selected protocol against a non-graph partner (\Cref{tab:fusioncontrol}). The objection is largely correct. VGG-16 delivers an EER reduction statistically indistinguishable from the Hybrid's, and its validation sweep selects $w{=}0.00$ --- it discards our CNN altogether, because VGG-16 is already the better-calibrated verifier. We therefore do \emph{not} claim the verification gain as evidence for graph modelling. The narrower claim the data does support is that, among the partners tested, only the graph branch improves ranking \emph{and} verification simultaneously: VGG-16 leaves Rank-1 unchanged and ProtoN costs 1.55 Rank-1 points to buy its lower EER.

\begin{table}[t]
  \centering
  \caption{Fusion control: the same validation-selected protocol with graph and non-graph partners. $\Delta$EER is the test-set reduction relative to the CNN alone (2.81\%), with a probe-level bootstrap 95\% CI. A non-graph partner matches the graph partner, so the verification gain is not graph-specific; only the Hybrid improves both metrics at once.}
  \label{tab:fusioncontrol}
  \small
  \setlength{\tabcolsep}{4pt}
  \begin{tabular}{lccccc}
    \toprule
    Partner & std.\ EER & sel.\ $w$ & fused R-1 & fused EER & $\Delta$EER (pt, 95\% CI) \\
    \midrule
    \textbf{Hybrid (graph)} & 1.88 & 0.95 & \textbf{95.95} & 1.17 & $+1.65$ $[+0.44,+2.48]$ \\
    ProtoN (graph) & 1.17 & 0.85 & 93.57 & \textbf{0.67} & $+2.14$ $[+0.50,+3.22]$ \\
    VGG-16 (\emph{non-graph}) & 1.23 & 0.00 & 95.12 & 1.23 & $+1.58$ $[+0.47,+2.63]$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
folds}. Exact McNemar tests on \emph{per-probe} single-split predictions (\texttt{scripts/compute\_real\_mcnemar.py}) confirm the CNN's advantage over both graph models is significant: CNN vs.\ Hybrid $n_{10}{=}43$, $n_{01}{=}13$, $p{=}7.3\times10^{-5}$; CNN vs.\ ProtoN $n_{10}{=}53$, $n_{01}{=}19$, $p{=}7.6\times10^{-5}$. The Hybrid and ProtoN are, however, \emph{statistically indistinguishable} ($n_{10}{=}38$, $n_{01}{=}34$, $p{=}0.72$), so their ordering in \Cref{tab:main} should not be read as a real difference.
...
\paragraph{Stage 2 --- Causal ablation, and a control that changes the conclusion.}
Qualitative maps are not evidence of causality, so we ablate the highest-importance evidence and measure the effect on identity. Crucially, the contrast one chooses decides the answer. Most explanation studies report \emph{top} vs.\ \emph{random}; we argue the honest control is \emph{top} vs.\ \emph{bottom} --- remove the same \emph{number} of nodes, ranked least-important instead of most --- because top and bottom both delete a coherent extreme of the ranking whereas a random subset is scattered, and that difference alone perturbs a graph.

For the \textbf{CNN branch} (\Cref{tab:ablation-cnn}, full 964-image test set) we compute Grad-CAM and mask the top-/random-/bottom-$k\%$ of $32\times32$ image regions. The attribution is faithful under \emph{both} contrasts: at $k{=}30$ masking the most important regions costs $5.0$ Rank-1 points versus $2.3$ for random and only $1.7$ for the least important, with flip rates $12.3\%$, $5.3\%$ and $5.7\%$. Removing what Grad-CAM calls important really does damage identity.

For the \textbf{GNN branch} (\Cref{tab:ablation}) the same test gives a \emph{negative} result, and we report it as such. Re-running on the full 964-graph test set (our earlier 120-graph subset left most probes without a gallery mate and is superseded here), graph Grad-CAM passes the conventional top-vs-random test --- $\Delta\cos$ $0.105$ vs.\ $0.048$ at $k{=}30$, CIs disjoint --- but \emph{fails} the top-vs-bottom control: removing the 30\% \emph{least} important nodes yields $\Delta\cos = 0.100$ $[0.094,0.107]$ against $0.105$ $[0.099,0.112]$ for the most important, overlapping CIs, and flip rates that are indistinguishable ($8.2\%$ vs.\ $8.6\%$; at $k{=}10$ the \emph{bottom} set flips slightly \emph{more}). The ranking therefore carries little causal signal beyond ``removing a coherent block of nodes hurts''. We draw two conclusions. First, for this model per-keypoint graph attributions should not be used to justify claims about \emph{which} muzzle regions drive identity. Second, and more generally, top-vs-random is too weak a control for graph explanations: it would have certified an attribution that the stricter contrast rejects.

\begin{table}[t]
  \centering
  \caption{Causal node ablation, \textbf{GNN branch} (GNN v3; full 964-graph test set; 95\% bootstrap CI on $\Delta\cos$). Graph Grad-CAM passes the conventional top-vs-\emph{random} test at every $k$, but \emph{fails} the stricter top-vs-\emph{bottom} control (italics): removing the least important nodes is nearly as damaging as removing the most, with overlapping CIs at $k{=}20,30$ and indistinguishable flip rates. Compare \Cref{tab:ablation-cnn}, where the CNN attribution passes both.}
  \label{tab:ablation}
  \small
  \begin{tabular}{lcc}
    \toprule
    ...
    \emph{bottom} 10\% & \emph{0.022} [0.021, 0.025] & \emph{4.4} \\
    \midrule
    top 20\%    & 0.064 [0.060, 0.069] & 6.1 \\
    random 20\% & 0.029 [0.027, 0.031] & 4.7 \\
    \emph{bottom} 20\% & \emph{0.059} [0.055, 0.064] & \emph{6.2} \\
    \midrule
    top 30\%    & 0.105 [0.099, 0.112] & 8.6 \\
    random 30\% & 0.048 [0.045, 0.052] & 6.2 \\
    \emph{bottom} 30\% & \emph{0.100} [0.094, 0.107] & \emph{8.2} \\
    \bottomrule
  \end{tabular}
\end{table}
...
consequence of it: the two graph operators consume the supplied topology very differently. The Dynamic EdgeConv is called \emph{without} the geometric \texttt{edge{\_}index} and rebuilds its neighbourhood graph in \emph{learned feature space} at every layer, so it never sees our edges. The GATv2 Topological Relation Module, by contrast, \emph{does} receive them (\texttt{hybrid{\_}model.py}, step 4) --- and is precisely the component our intervention randomises. The correct reading of \Cref{tab:pathway} is therefore not that the geometry is \emph{overridden} but the sharper, less comfortable one: \textbf{the relation module that consumes it contributes essentially nothing to identity.} The remaining two null results are structural rather than empirical: the default configuration exposes no edge-attribute pathway, and mean+max pooling is permutation-invariant, so zeroing edge attributes and shuffling keypoint indices could not have moved the metric. In other words, on clean in-domain data the Hybrid's identity signal is carried almost entirely by the CNN texture sampled at keypoints and by feature-space message passing --- not by the input muzzle \emph{geometry}.
...
Together these three stages make the explainability claims falsifiable and reproducible --- and two of the three return negative results, which is precisely the point of running them --- the
...
ranking, we \emph{bound} the graph branch's value with two controls. A non-graph baseline fused into the same protocol matches the Hybrid's EER reduction (\Cref{tab:fusioncontrol}), so the verification gain is not graph-specific; what is specific is only that the graph partner improves ranking and verification together. And a pathway intervention shows, as an instructive negative result, that even this comes from the keypoint-sampled CNN features and feature-space message passing rather than from the explicit geometric graph we construct --- the GATv2 module that consumes that geometry can be fed random edges at almost no cost. We show
...
low mutual rank agreement, and graph Grad-CAM additionally fails our top-vs-bottom ablation control (\Cref{tab:ablation}); per-keypoint graph importance should therefore be treated as \emph{unvalidated} for this model, not merely indicative. The CNN's image-space attributions do pass the same control.
